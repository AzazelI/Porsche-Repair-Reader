...
${text}`;
    ...
    // ==========================================
    // OBD-II & PIWIS TELEMETRY COCKPIT CONTROLLER
    // ==========================================
    
    // UI Navigation nodes
    const tabReader = document.getElementById("tab-reader");
    const tabObd = document.getElementById("tab-obd");
    const obdSection = document.getElementById("obd-section");
    const obdStatusBadge = document.querySelector(".obd-status-badge");
    
    // OBD Control nodes
    const tpsSlider = document.getElementById("obd-tps-slider");
    const tpsSliderVal = document.getElementById("obd-tps-slider-val");
    const pasmSlider = document.getElementById("obd-pasm-slider");
    const pasmSliderVal = document.getElementById("obd-pasm-slider-val");
    const exhaustToggle = document.getElementById("obd-exhaust-toggle");
    
    // Action buttons
    const btnObdAccelerate = document.getElementById("btn-obd-accelerate");
    const btnObdTriggerFault = document.getElementById("btn-obd-trigger-fault");
    const btnObdClearDtc = document.getElementById("btn-obd-clear-dtc");
    
    // Log & Terminal viewports
    const canBusTerminal = document.getElementById("can-bus-terminal");
    const obdDtcLog = document.getElementById("obd-dtc-log");
    ...
    // 1. Navigation toggle
    if (tabReader && tabObd && obdSection) {
        tabReader.addEventListener("click", () => {
            tabReader.classList.add("active");
            tabObd.classList.remove("active");
            obdSection.classList.add("hidden");
            
            // Re-show reader panels
            const hasDashboardData = !dashboardSection.classList.contains("hidden");
            if (hasDashboardData) {
                dashboardSection.classList.remove("hidden");
            } else {
                landingContainer.classList.remove("hidden");
            ...
            // Stop sound and stream loops to conserve resources
            stopEngineSound();
        ...
        tabObd.addEventListener("click", () => {
            tabObd.classList.add("active");
            ...
            obdSection.classList.remove("hidden");
            
            // Sync with backend on enter
            syncWithBackend();
            
            // Start stream loop and audio synthesis
            startCANStreamLoop();
            startBackendSyncLoop();
        });
    }
    
    // 2. SVG Gauge Renderer
    function renderObdGauges() {
        // RPM needle rotation: map 0-9000 RPM -> -120deg to 120deg
        const rpmNeedle = document.getElementById("obd-rpm-needle-group");
        const rpmValSpan = document.getElementById("obd-rpm-val");
        const rpmGearSpan = document.getElementById("obd-rpm-gear");
        
        if (rpmNeedle) {
            const angle = -120 + (obdState.rpm / 9000) * 240;
            rpmNeedle.style.transform = `rotate(${angle}deg)`;
        }
        if (rpmValSpan) {
            rpmValSpan.textContent = Math.round(obdState.rpm);
        }
        if (rpmGearSpan) {
            rpmGearSpan.textContent = activeGear;
        }
        ...
        // Temperature needle rotation: map 0-140 °C -> -120deg to 120deg
        const tempNeedle = document.getElementById("obd-temp-needle-group");
        const tempValSpan = document.getElementById("obd-temp-val");
        
        if (tempNeedle) {
            const angle = -120 + (obdState.temp / 140) * 240;
            tempNeedle.style.transform = `rotate(${angle}deg)`;
        }
        if (tempValSpan) {
            tempValSpan.textContent = Math.round(obdState.temp);
        }
    }
    
    // 3. User Controls Listeners
    if (tpsSlider) {
        tpsSlider.addEventListener("input", (e) => {
            if (isAccelerating) return; // Ignore input during launch sequence
            
            const tps = parseInt(e.target.value, 10);
            obdState.tps = tps;
            tpsSliderVal.textContent = `${tps}%`;
            
            // Map throttle position to Engine RPM: 800 (idle) to 7200 RPM
            obdState.rpm = 800 + (tps / 100) * 6400;
            
            // Dynamic gear calculations
            if (tps > 0) {
                activeGear = "1";
                // simple speed simulation based on TPS
                obdState.speed = (tps / 100) * 80;
            } else {
                activeGear = "N";
                obdState.speed = 0;
            }
            ...
            // Web Audio sound adjustments
            if (tps > 0) {
                if (!audioCtx) startEngineSound();
                updateEngineSound(obdState.rpm / 1000);
            } else {
                stopEngineSound();
            }
            
            // Throttle posting metrics to backend
            postMetricsToBackend();
        });
    }
    
    if (pasmSlider) {
        pasmSlider.addEventListener("input", (e) => {
            const pasm = parseInt(e.target.value, 10);
            obdState.pasm = pasm;
            pasmSliderVal.textContent = `${pasm}%`;
            postMetricsToBackend();
        });
    }
    ...
    if (exhaustToggle) {
        exhaustToggle.addEventListener("change", (e) => {
            ...
            obdState.exhaust_valve = val;
            
            // Update audio synth parameters dynamically
            if (audioCtx && filterNode) {
                const targetFreq = val ? 1200 : 240;
                filterNode.frequency.setValueAtTime(targetFreq, audioCtx.currentTime);
                playBeep(val ? 1100 : 440, 0.05);
            }
            postMetricsToBackend();
        });
    }
    
    // 4. API Backend Communicators
    function postMetricsToBackend() {
        const url = apiUrlInput.value.trim() || DEFAULT_API_URL;
        fetch(`${url}/api/obd/metrics`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                rpm: obdState.rpm,
                speed: obdState.speed,
                temp: obdState.temp,
                tps: obdState.tps,
                pasm: obdState.pasm,
                exhaust_valve: obdState.exhaust_valve
            })
        })
        .catch(err => console.warn("Failed to post OBD telemetry:", err));
    }
    ...
    function syncWithBackend() {
        ...
        .then(res => res.json())
        .then(data => {
            obdState.rpm = data.rpm || 800;
            obdState.speed = data.speed || 0;
            obdState.temp = data.temp || 90;
            obdState.tps = data.tps || 0;
            obdState.pasm = data.pasm || 30;
            obdState.exhaust_valve = data.exhaust_valve || false;
            obdState.dtcs = data.dtcs || [];
            
            // Update UI widgets
            if (tpsSlider) {
                tpsSlider.value = obdState.tps;
                tpsSliderVal.textContent = `${obdState.tps}%`;
            }
            if (pasmSlider) {
                pasmSlider.value = obdState.pasm;
                pasmSliderVal.textContent = `${obdState.pasm}%`;
            }
            if (exhaustToggle) {
                exhaustToggle.checked = obdState.exhaust_valve;
            }
            
            renderDtcUI();
            renderObdGauges();
        })
        .catch(err => console.warn("Failed to sync with OBD backend:", err));
    }
    
    function renderDtcUI() {
        if (!obdDtcLog) return;
        obdDtcLog.innerHTML = "";
        
        if (obdState.dtcs && obdState.dtcs.length > 0) {
            obdStatusBadge.classList.add("fault-mode");
            obdStatusBadge.innerHTML = `<span class="pulse-dot red"></span> Fault Alert (ხარვეზი!)`;
            
            obdState.dtcs.forEach(d => {
                const div = document.createElement("div");
                div.className = "dtc-item error-status";
                div.innerHTML = `
                    <i class="fa-solid fa-triangle-exclamation"></i>
                    <div>
                        <span class="dtc-code-badge">${d.code}</span>
                        <span><strong>${d.description}</strong></span>
                    </div>
                `;
                obdDtcLog.appendChild(div);
            });
        } else {
            obdStatusBadge.classList.remove("fault-mode");
            obdStatusBadge.innerHTML = `<span class="pulse-dot green"></span> Connected (კავშირი აქტიურია)`;
            
            const div = document.createElement("div");
            div.className = "dtc-item ok-status";
            div.innerHTML = `
                <i class="fa-solid fa-circle-check"></i>
                <span>სისტემა სუფთაა (DTC codes: 0)</span>
            `;
            obdDtcLog.appendChild(div);
        }
    }
    
    // 5. Simulated 100ms CAN-bus Stream
    function startCANStreamLoop() {
        if (canStreamInterval) clearInterval(canStreamInterval);
        
        canBusTerminal.textContent = "Connecting to OBD-II CAN controller...\nCAN Frame buffer initialized...\nStreaming active CAN bus frames:\n";
        
        const backgroundIds = ["0x00000100", "0x00000120", "0x000001B0", "0x00000320", "0x00000450", "0x000005F2"];
        
        canStreamInterval = setInterval(() => {
            if (obdSection.classList.contains("hidden")) return;
            
            // Format simulated CAN frames locally at high speed
            const now = new Date();
            const timestamp = now.toLocaleTimeString() + "." + String(now.getMilliseconds()).padStart(3, "0");
            
            // Prepare some formatted output logs
            let logLine = "";
            const isDiagnoseFrame = Math.random() > 0.6;
            
            if (isDiagnoseFrame) {
                // Generate a diagnostic metrics frame
                const choice = Math.floor(Math.random() * 4);
                if (choice === 0) {
                    const rpm_raw = intToHexBytes(Math.round(obdState.rpm * 4), 2);
                    logLine = `${timestamp}  18DAF110  [8]  04 41 0C ${rpm_raw} 00 00  (Engine RPM: ${Math.round(obdState.rpm)})`;
                } else if (choice === 1) {
                    const speed_hex = intToHexBytes(Math.round(obdState.speed), 1);
                    logLine = `${timestamp}  18DAF110  [8]  03 41 0D ${speed_hex} 00 00 00 00  (Vehicle Speed: ${Math.round(obdState.speed)} km/h)`;
                } else if (choice === 2) {
                    const temp_hex = intToHexBytes(Math.round(obdState.temp + 40), 1);
                    logLine = `${timestamp}  18DAF110  [8]  03 41 05 ${temp_hex} 00 00 00 00  (Coolant Temp: ${Math.round(obdState.temp)} °C)`;
                } else {
                    const tps_hex = intToHexBytes(Math.round(obdState.tps * 255 / 100), 1);
                    logLine = `${timestamp}  18DAF110  [8]  03 41 11 ${tps_hex} 00 00 00 00  (Throttle Position: ${Math.round(obdState.tps)}%)`;
                }
            } else {
                // Generate background CAN noise frame
                const cid = backgroundIds[Math.floor(Math.random() * backgroundIds.length)];
                const data = Array.from({length: 8}, () => Math.floor(Math.random() * 256).toString(16).toUpperCase().padStart(2, "0")).join(" ");
                logLine = `${timestamp}  ${cid.replace("0x", "")}  [8]  ${data}  (Background CAN Broadcast)`;
            }
            
            // Append and scroll terminal
            const lines = canBusTerminal.textContent.split("\n");
            if (lines.length > 100) {
                lines.shift();
            }
            lines.push(logLine);
            canBusTerminal.textContent = lines.join("\n");
            canBusTerminal.scrollTop = canBusTerminal.scrollHeight;
            
        }, 100);
    }
    
    function intToHexBytes(val, numBytes) {
        let hex = val.toString(16).toUpperCase().padStart(numBytes * 2, "0");
        let bytes = [];
    ...
    // 6. Accelerate Sweep / Launch Control Run simulation
    if (btnObdAccelerate) {
        btnObdAccelerate.addEventListener("click", () => {
            if (isAccelerating) return;
            
            isAccelerating = true;
            btnObdAccelerate.disabled = true;
            btnObdAccelerate.style.opacity = "0.5";
            
            // Lock other sliders
            if (tpsSlider) tpsSlider.disabled = true;
            if (pasmSlider) pasmSlider.disabled = true;
            if (exhaustToggle) exhaustToggle.disabled = true;
            
            // Automatically switch Exhaust Valve to ON
            obdState.exhaust_valve = true;
            if (exhaustToggle) exhaustToggle.checked = true;
            
            // Sequence start sound
            startEngineSound();
            if (filterNode) filterNode.frequency.setValueAtTime(1200, audioCtx.currentTime);
            
            // Stage 1: Pre-Stage Launch Control RPM Rev to 8600 RPM
            let tpsVal = 0;
            let rpmVal = 800;
            activeGear = "N";
            ...
            let stageTimer = setInterval(() => {
                tpsVal += 5;
                rpmVal = 800 + (tpsVal / 100) * 7800; // climbs to 8600
                
                obdState.tps = Math.min(100, tpsVal);
                obdState.rpm = Math.min(8600, rpmVal);
                if (tpsSlider) {
                    tpsSlider.value = obdState.tps;
                    tpsSliderVal.textContent = `${obdState.tps}%`;
                }
                ...
                if (tpsVal >= 100) {
                    ...
                    // Bounce launch limiter 6 times! (tat-tat-tat-tat!)
                    let bounceCount = 0;
                    let bounceTimer = setInterval(() => {
                        obdState.rpm = obdState.rpm === 8600 ? 8100 : 8600;
                        updateEngineSound(obdState.rpm / 1000);
                        renderObdGauges();
                        playBeep(70, 0.04);
                        ...
                        if (bounceCount > 6) {
                            ...
                            // Stage 2: LAUNCH GO! Sweep through gears 1 to 4!
                            executeLaunchDriveSequence();
                        }
                    }, 80);
                }
            }, 30);
    ...
    function executeLaunchDriveSequence() {
        let speedVal = 0;
        let rpmVal = 8000;
        activeGear = "1";
        ...
        let launchTimer = setInterval(() => {
            // Speed climbs
            speedVal += 2.5;
            obdState.speed = Math.round(speedVal);
            
            // RPM climbs up to redline shifts
            if (activeGear === "1") {
                rpmVal = 5000 + (speedVal / 50) * 3500; // 5000 -> 8500
                if (speedVal >= 50) {
                    // Shift to 2nd gear
                    activeGear = "2";
                    rpmVal = 5200;
                    playBeep(400, 0.08); // shift pop
                }
            } else if (activeGear === "2") {
                rpmVal = 5200 + ((speedVal - 50) / 40) * 3300; // 5200 -> 8500
                if (speedVal >= 90) {
                    // Shift to 3rd gear
                    activeGear = "3";
                    rpmVal = 5400;
            ...
            } else if (activeGear === "3") {
                rpmVal = 5400 + ((speedVal - 90) / 40) * 3100; // 5400 -> 8500
                if (speedVal >= 130) {
                    // Shift to 4th gear
                    activeGear = "4";
                    rpmVal = 5600;
            ...
            } else if (activeGear === "4") {
                rpmVal = 5600 + ((speedVal - 130) / 60) * 2900; // 5600 -> 8500
                if (speedVal >= 190) {
                    activeGear = "5";
                    rpmVal = 5800;
                    playBeep(400, 0.08);
                }
            } else if (activeGear === "5") {
                rpmVal = 5800 + ((speedVal - 190) / 60) * 2200; // 5800 -> 8000
                if (speedVal >= 250) {
                    // Settle at top speed
                    clearInterval(launchTimer);
                    
                    // Decelerate back to normal
                    decelerateSimulationSequence();
                }
            }
            
            obdState.rpm = Math.min(8800, rpmVal);
            updateEngineSound(obdState.rpm / 1000);
            renderObdGauges();
            postMetricsToBackend();
    ...
    function decelerateSimulationSequence() {
        let speedVal = obdState.speed;
        let rpmVal = obdState.rpm;
        activeGear = "D";
        
        let decelTimer = setInterval(() => {
            speedVal -= 4;
            rpmVal -= 100;
            obdState.tps = Math.max(0, obdState.tps - 5);
            ...
            obdState.speed = Math.max(0, speedVal);
            obdState.rpm = Math.max(800, rpmVal);
            
            updateEngineSound(obdState.rpm / 1000);
            renderObdGauges();
            postMetricsToBackend();
            
            if (speedVal <= 0 && rpmVal <= 800) {
                ...
                btnObdAccelerate.disabled = false;
                btnObdAccelerate.style.opacity = "1";
                if (tpsSlider) tpsSlider.disabled = false;
                if (pasmSlider) pasmSlider.disabled = false;
                if (exhaustToggle) exhaustToggle.disabled = false;
    ...
    // 7. Fault simulation Triggers
    if (btnObdTriggerFault) {
        btnObdTriggerFault.addEventListener("click", () => {
            const url = apiUrlInput.value.trim() || DEFAULT_API_URL;
            btnObdTriggerFault.disabled = true;
            
            // Call FastAPI to trigger P0300
            fetch(`${url}/api/obd/dtc/trigger`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({
                    code: "P0300",
                    description: "Random/Multiple Cylinder Misfire Detected (ცილინდრებში აალების გამოტოვება)"
                })
            })
            .then(res => res.json())
            .then(data => {
                btnObdTriggerFault.disabled = false;
                // Update local status
                obdState.dtcs = data.dtcs || [];
                
                // Set engine temp higher to simulate diagnostic symptom
                obdState.temp = 108; // warning red area!
                
                renderDtcUI();
                renderObdGauges();
                playBeep(330, 0.4);
                
                // Post temp update to backend
                postMetricsToBackend();
            })
            .catch(err => {
                btnObdTriggerFault.disabled = false;
    ...
    if (btnObdClearDtc) {
        btnObdClearDtc.addEventListener("click", () => {
            const url = apiUrlInput.value.trim() || DEFAULT_API_URL;
            btnObdClearDtc.disabled = true;
            
            // Call FastAPI to clear DTCs
            fetch(`${url}/api/obd/dtc/clear`, {
                method: "POST"
            })
            .then(res => res.json())
            .then(data => {
                btnObdClearDtc.disabled = false;
                obdState.dtcs = [];
                obdState.temp = 90; // back to normal!
